if (self.atApplication === undefined || self.atAplication === null)
  self.atApplication = {};
...
atApplication.namespace = (object, address, createOnSearch) => {
...
atApplication.ls = function (listOfStuff) {
...
atApplication.getTimestampNanos = () => {
  return (
    BigInt(
      Math.round(
        (performance.now() -
          atApplication.getTimestampNanos.startTime.relative) *
          100000
      )
...
atApplication.namespace(atApplication.getTimestampNanos, "startTime").absolute =
  BigInt(Date.now()) * 1000000n;
atApplication.namespace(atApplication.getTimestampNanos, "startTime").relative =
  performance.now();
...
atApplication.test.namespace = atApplication.namespace;
{
  ...
  atRoot.isObject = item => {
    return item === Object(item);
  };
  atRoot.isObject_notArray = item => {
    return atRoot.isObject(item) && !Array.isArray(item);
  };
  atRoot.isArray = item => {
    return Array.isArray(item);
  };
  atRoot.isString = item => {
    return Object.prototype.toString.call(item) === "[object String]";
  };
  atRoot.isFunction = item => {
  ...
  atRoot.namespace = function (object, address, defaultList, checkExists) {
    ...
    if (object === undefined || object === null || !atRoot.isObject(object)) {
      let errorMessage =
        "@: ERROR: atSrc: namespace: object : this Object is not a valid namespace root object";
    ...
    if (address === undefined || !atRoot.isString(address)) {
      let errorMessage =
        "@: ERROR: atSrc: namespace: address : this address is not a valid address string";
      atRoot.ls(errorMessage, { object: object, address: address });
    ...
    for (var wayPoint of addressList) {
      ...
      if (!current.hasOwnProperty(wayPoint)) {
        ...
        if (!atRoot.regex.startsWithToReturn.test(toEval))
          toEval = "toReturn = " + toEval + ";";
  ...
  atRoot.namespace.isNotFound = function (value, address) {
    ...
    return (
      atRoot.isObject_notArray(value) &&
  ...
  atRoot.namespace.traverse = function (traveller) {
    ...
    if (object === undefined || object === null || !atRoot.isObject(object)) {
      let errorMessage =
        "@: ERROR: atSrc: namespace: getIfExists: object : this Object is not a valid namespace root object";
      atRoot.ls(errorMessage, { object: object, address: address });
      throw new Error(errorMessage);
    }
    ...
    if (
      address === undefined ||
      (!atRoot.isString(address) && isNaN(address))
    ) {
      let errorMessage =
        "@: ERROR: atSrc: namespace: getIfExists: address : this address is not a valid address string";
      atRoot.ls(errorMessage, { object: object, address: address });
      throw new Error(errorMessage + ": " + address);
    ...
    for (traveller.addressComponent of traveller.addressList) {
      ...
      traveller.finalAddressComponent =
        traveller.index >= traveller.addressListLength - 1;
      traveller.func(traveller);
      if (traveller.returnNow === true) return traveller.toReturn;
      traveller.current = traveller.next;
    }
    throw new Error(
      "@: ERROR: atSrc: namespace: traverse: loop terminated without returning - this should never happen"
    );
  };
  atRoot.namespace.getIfExists_callback = function (
    object,
    address,
    found,
    notFound
  ...
  atRoot.namespace.getIfExists = function (object, address, traveller) {
    ...
    traveller.func = traveller => {
      if (
        traveller.next === undefined ||
        traveller.next === atRoot.namespace.NotFound
      ) {
  ...
  atRoot.namespace.getMustExist = function (object, address, options) {
    ...
    if (toReturn === atRoot.namespace.NotFound) {
      let debuggingOutput =
        ...
        `{"propertyNotFound": {"address": ${util.inspect(address).slice(0, 500)}, "object": ${util.inspect(object).slice(0, 2000)} } }`;
  ...
  atRoot.namespace.destructureMustExist = function (
    object,
    destructureDefinition,
    options
  ) {
    let toReturn = {};
    for (let [key, value] of Object.entries(destructureDefinition)) {
      atRoot.namespace.setValue(
        toReturn,
        key,
        atRoot.namespace.getMustExist(object, value, options)
      );
    }
    return toReturn;
  };
  ...
  atRoot.namespace.leafNode = function (object, address, leafNode) {
    ...
    try {
      toReturn = atRoot.namespace.setValue(
        object,
        address,
        leafNode,
        traveller
      ...
      if (
        error.message.startsWith(
          "@: atSrc: namespace: setValue: cannot overwrite existing value:"
        )
      ) {
  ...
  atRoot.namespace.setValue = function (object, address, value, traveller) {
    ...
    if (address === null)
      throw new Error(
        "@: atSrc: namespace: setValue: setting the address to null implies setting the containing object, which is impossible: " +
          address
      );
      ...
      if (!traveller.finalAddressComponent) {
        if (traveller.next === undefined) {
          ...
          if (traveller.ignoreErrors === true) {
            ...
            throw new Error(
              "@: atSrc: namespace: setValue: there is no valid object hierarchy to " +
                traveller.address
            );
          }
        }
        ...
        if (
          traveller.overwrite !== true &&
          traveller.ignoreErrors !== true &&
          traveller.next !== undefined
        ) {
          throw new Error(
            "@: atSrc: namespace: setValue: cannot overwrite existing value: " +
  ...
  atRoot.namespace.extend = function (
    extendIntoThisObject,
    extendFromThisObject,
    traveller
  ) {
  ...
  atRoot.namespace.flatten = function (
    objectToFlatten,
    currentNamespace,
    toReturn
  ) {
    ...
    if (atRoot.isObject(objectToFlatten)) {
      for (let [namespaceItem, recurseIntoThisObject] of Object.entries(
        objectToFlatten
      )) {
        let targetNamespace;
        ...
        atRoot.namespace.flatten(
          recurseIntoThisObject,
          targetNamespace,
          toReturn
        );
      }
  ...
  atRoot.namespace.extendObjectWithVennComplementOfTwo = function (
    object,
    vennData,
    optionsDict
  ) {
    vennData.inOneNotTwo.map(namespaceString => {
      namespace.leafNode(
        object,
        namespaceString,
        namespace.getMustExist(vennData.complementOfTwo, namespaceString)
      );
      //namespace.setValue(object, namespaceString, namespace.getMustExist(vennData.complementOfTwo, namespaceString), optionsDict)
    });
  };
  atRoot.namespace.buildStateFromDeepCopy = function (
    persistentDataAndVennData
  ) {
    let persistableStateToAssign = JSON.parse(
      JSON.stringify(persistentDataAndVennData.travellerState_persistable)
    );
    ...
    namespace.extendObjectWithVennComplementOfTwo(
      fullStateToAssign,
      undoTravellerToThisState.vennData,
      { overwrite: true }
    );
    namespace.extend(fullStateToAssign, persistableStateToAssign, {
      ignoreErrors: true,
    });
  ...
  atRoot.namespace.deepCopyTraveller = function (copyThisTraveller) {
    ...
    let travellerState_original = {
      traveller: namespace.getMustExist(copyThisTraveller, "traveller"),
    };
    let travellerState_persistable = {
      traveller: JSON.parse(JSON.stringify(copyThisTraveller.traveller)),
    };
    let vennData = atRoot.namespace.venn(
      travellerState_original,
      travellerState_persistable
    );

    debugger;

    return {
      travellerState_persistable: travellerState_persistable,
      vennData: vennData,
    };
  };
  ...
  atRoot.namespace.venn = function (object1, object2, fullAddress, toReturn) {
    if (!atRoot.isObject(object1)) {
      throw new Error(
        "@: atSrc: namespace.venn: object1 parameter contains a primitive, not an object"
      );
    }
    ...
    for (const [key, value] of Object.entries(object1)) {
      ...
      if (
        !atRoot.isObject(object2) ||
        (!key) in object2 ||
        atRoot.isFunction(value)
      ) {
        toReturn.inOneNotTwo.push(recurseNewFullAddress);
        toReturn.changed = true;
        namespace.setValue(
          toReturn.complementOfTwo,
          recurseNewFullAddress,
          value
        );
        //////////////////////// CONTINUE ///////////////////
        continue;
    ...
    if (object2KeyList !== null && object2KeyList.length > 0) {
      toReturn.inTwoNotOne.push(
        ...object2KeyList.map(keyNotFoundInOne => {
          let fullAddressInTwo = fullAddressPrefix + keyNotFoundInOne;
          namespace.setValue(
            toReturn.complementOfOne,
            fullAddressInTwo,
            object2[keyNotFoundInOne]
          );
          toReturn.changed = true;
    ...
    if (fullAddress === null)
      if (toReturn.changed !== true) toReturn = undefined;

    return toReturn;
  };
  atRoot.namespace.deepCopy = function (
    copyThisObject,
    fullDottedAddressString,
    runtimeNamespaceList
  ) {
    //, objectToReturn)
    if (!atRoot.isObject(copyThisObject)) {
      // is a primitive?)
    ...
    if (
      fullDottedAddressString === undefined ||
      fullDottedAddressString === null
    ) {
      fullDottedAddressString = "";
    }

    if (runtimeNamespaceList === undefined || runtimeNamespaceList === null) {
      runtimeNamespaceList = atRoot.namespace.getIfExists(
        copyThisObject,
        "traveller.routerAsAService.nestTravellerFlow.runtimeNamespaceList"
      );
      if (atRoot.namespace.isNotFound(runtimeNamespaceList))
        runtimeNamespaceList = [];
    }
    ...
    for (let [key, value] of Object.entries(copyThisObject)) {
      if (runtimeNamespaceList.contains(fullDottedAddressString + dotKey)) {
        ...
        objectToReturn[key] = atRoot.namespace.deepCopy(
          value,
          fullDottedAddressString + "." + key,
          runtimeNamespaceList
        );
      }
    }
  ...
  atRoot.namespace.toEval_localVars_store = function (
    objectName,
    address,
    localVarNameList,
    localVarNameDict
  ...
  atRoot.namespace.toEval_localVars_load = function (
    objectName,
    address,
    localVarNameList,
    localVarNameDict
  ...
  atRoot.namespace.contains = function (object, address, listOfKeys) {
    ...
    if (Array.isArray(checkThisObject)) {
      for (var i = 0, len = listOfKeys.length; i < len; i++) {
        if (checkThisObject.indexOf(listOfKeys[i]) == -1) return false;
      }
    } else {
      for (var i = 0, len = listOfKeys.length; i < len; i++) {
        if (!checkThisObject.hasOwnProperty(listOfKeys[i])) return false;
      }
    }
  ...
  atRoot.namespace.joinWordList_helper = function (separator, ...toJoin) {
    if (!atRoot.isString(separator))
      throw new Error(
        "@: atSrc: namespace.joinWordList_helper: separator must be a string"
      );
  ...
  atRoot.namespace.compareSlice = function (
    namespaceToCheck,
    compareTo,
    sliceParams,
    options
    ...
    try {
      ...
      if (atRoot.isString(namespaceToCheck))
        namespaceToCheck = namespaceToCheck.split(".");
      ...
      for (let i = sliceToCheck.length - 1; i > -1; i--) {
        if (sliceToCheck[i] !== sliceToCompare[i] && sliceToCompare[i] !== "*")
          return false;
      }
      return true;
    } catch (error) {
  ...
  atRoot.namespace.tween = function (namespaceToTween) {
    if (!atRoot.isString(namespaceToTween)) {
      return undefined;
    }
    return namespaceToTween.split(".").join(".children.");
  };
  atRoot.namespace.iterate = function* (namespaceToIterate, tweenWith) {
    // debugger;
    ...
    while (namespaceToIterateList.length > 0) {
      ...
      yield {
        ...
        remainingNamespaceList: namespaceToIterateList,
      };
    }
    return;
  };
  atRoot.namespace.iterateFragment = function* (namespaceToIterate, tweenWith) {
    for (let toYield of atRoot.namespace.iterate(
      namespaceToIterate,
      tweenWith
    )) {
  ...
  atRoot.namespace.iterateItem = function* (namespaceToIterate) {
    for (let toYield of atRoot.namespace.iterate(
      namespaceToIterate,
      tweenWith
    )) {
  ...
  atRoot.namespace.isRootOf = function (rootNamespace, targetNamespace) {
    let removeTargetNamespace = targetNamespace.replace(rootNamespace, "");
    return (
      removeTargetNamespace === "" || removeTargetNamespace.startsWith(".")
    );
  };
}
...
atApplication.isObject = item => {
  return item === Object(item);
};
atApplication.isObject_notArray = item => {
  return isObject(item) && !Array.isArray(item);
};
atApplication.isArray = item => {
  return Array.isArray(item);
};
atApplication.isString = item => {
  return Object.prototype.toString.call(item) === "[object String]";
};
atApplication.isFunction = item => {
  return Object.prototype.toString.call(item) === "[object Function]";
};
atApplication.isInteger = item => {
...
atApplication.namespace.setValue(
  atApplication,
  "regexCollection.urlWithProtocol_regex",
  /^http[s]?:i\/\//
);

atApplication.namespace.setValue(
  atApplication,
  "functionCollection.concatBaseURL",
  function (baseURL, targetURL) {
    let prependToURL = baseURL;
    let separator = "";
    if (
      atApplication.regexCollection.urlWithProtocol_regex.test(targetURL) ===
      true
    ) {
      prependToURL = "";
...
atApplication.toEval_installAtApplicationFunctionsInLocalNamespace = `   var AT = atApplication;
