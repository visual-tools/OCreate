this.tabBlock_except = function(except)
{
  ...
  for (let element of $document.find(
      elementsWhichDefaultToHavingTabIndex_cssSelector,
    ))
  {
    element = $(element);
...
this.phaseContainer = function phaseContainer(
  referenceObject,
  containingElement,
...
)
{
  O.create(
    [
      ...
      [
        ...
        [
          "button.backButton.buttonLeaf.buttonLeafReversed.positionAbsolute",
          "@back",
        ],
      ],
    ],
  ...
  referenceObject.closeButton.on("click", () =>
  {
    debugger;
    ...
    setTimeout(() =>
    {
      referenceObject.phaseContainer_container.remove();
    }, 200);
  ...
  referenceObject.nextButton.on("click", () =>
  {
    loadPhaseCollection.next();
  });
  referenceObject.backButton.on("click", () =>
  {
  ...
  loadPhaseCollection.load = (newIndex) =>
  {
    loadPhaseCollection.index = newIndex;
    loadPhaseCollection.currentPhase =
      phaseCollection[loadPhaseCollection.index];
    setTimeout(() =>
    {
      loadPhaseCollection.currentPhase.load();
    }, 100);

    if (loadPhaseCollection.index === 0)
      referenceObject.backButton.toggleClass("displayNone", true);
    else if (loadPhaseCollection.index > 0)
      referenceObject.backButton.toggleClass("displayNone", false);
  ...
  loadPhaseCollection.buttonState = (buttonName, newState) =>
  {
    if (newState === undefined)
      newState = namespace.leafNode(
        loadPhaseCollection,
        "currentPhase.buttonStateDict." + buttonName,
        "disabled",
      );
    referenceObject[buttonName].attr(
      "disabled",
      newState !== "enabled" ? "disabled" : null,
    );
    namespace.setValue(
      loadPhaseCollection,
      "currentPhase.buttonStateDict." + buttonName,
  ...
  loadPhaseCollection.nextButtonState = (newState) =>
    loadPhaseCollection.buttonState("nextButton", newState);
  loadPhaseCollection.backButtonState = (newState) =>
    loadPhaseCollection.buttonState("backButton", newState);
  ...
  let directionOfTransferPhase = {
    load: () =>
    {
      if (directionOfTransferPhase.loaded !== true)
      {
        ...
        directionOfTransferPhase.sendCreditToDiv.on("click", (event) =>
        {
          debugger;
          if (directionOfTransferPhase.direction !== "send")
          {
            directionOfTransferPhase.sendCreditTo.toggleClass(
              "selected", true);
            directionOfTransferPhase.getCreditFrom.toggleClass(
              "selected",
              false,
            );
        ...
        directionOfTransferPhase.getCreditFromDiv.on("click", (event) =>
        {
          debugger;
  ...
  let targetAccountPhase = {
    load: () =>
    {
      if (targetAccountPhase.loaded !== true)
      {
        ...
        for (const [ledgerNodeName, ledgerInstanceData] of Object.entries(
            userIDListByLedgerNameDict,
          ))
        {
          if (
            nodes.browserState.location.href.includes("stage1") &&
            ledgerNodeName !== "mcsstage1"
          )
          {
            continue;
          }
          else if (
            nodes.browserState.location.href.includes("stage2") &&
            ledgerNodeName !== "mcsstage2"
          )
          {
            continue;
          }
          const ledgerMenuItemName =
            shortDisplayNameByLedgerNameDict[ledgerNodeName];
          let ledgerOptionElement = $(
            `<option value="${ledgerNodeName}">${ledgerMenuItemName}</option>`,
          );
          if (ledgerNodeName === currentLedgerNodeName)
            ledgerOptionElement.attr("selected", "selected");
          targetAccountPhase.selectTargetLedgerSelect.append(
            ledgerOptionElement,
          );
        }

        targetAccountPhase.selectTargetLedgerSelect.on("change", (
          event) =>
        {
          debugger;
          ...
          targetAccountPhase.selectTargetUserIDSelect.append(
            userIDOptionElement,
          );
          for (const accountID of userIDListByLedgerNameDict[
              targetAccountPhase.selectTargetLedgerSelect.val()
            ].userIDList)
          {
            if (
              !theseAccountsCannotBeCounterPartiesList.includes(
                accountID) &&
              (accountID !== userEmailAddress ||
                targetAccountPhase.selectTargetLedgerSelect.val() !==
                currentLedgerNodeName)
            )
            {
          ...
          targetAccountPhase.selectTargetUserIDSelect.trigger(
            "change");
        });
        targetAccountPhase.selectTargetLedgerSelect.trigger("change");

        targetAccountPhase.selectTargetUserIDSelect.on("change", (
          event) =>
        {
          debugger;
          if (
            targetAccountPhase.selectTargetUserIDSelect.val() !==
            "noUserIDSelected"
          )
          {
            loadPhaseCollection.nextButtonState("enabled");
    ...
    load: () =>
    {
      if (quantityAndDescriptionPhase.loaded !== true)
      {
                ...
                [
                  [
                    ...
                    [
                      [
                        ".rowTitle.flex_grow1",
                        "@amount to " + directionOfTransferPhase
                        .direction,
                      ],
        ...
        quantityAndDescriptionPhase.quantityOfCreditAutoNumeric =
          new AutoNumeric(
            quantityAndDescriptionPhase.quantityOfCredit.get(0),
            "0.0",
            {
              currencySymbol: "Hr ",
              minimumValue: 0,
              emptyInputBehaviour: "Hr 0.00",
              decimalPlaces: 1,
            },
          );
        ...
        let checkEnableNext = () =>
        {
          ...
          if (
            ...
            .get() !==
            "0" &&
            quantityAndDescriptionPhase.description.val() !== ""
          )
          {
            loadPhaseCollection.nextButtonState("enabled");
          }
        ...
        quantityAndDescriptionPhase.quantityOfCredit.on("keyup", (
          event) =>
        {
        ...
        quantityAndDescriptionPhase.description.on("keyup", (event) =>
        {
          checkEnableNext();
        });
  ...
  let transactionSummaryManager = namespace(
    newTransaction,
    "transactionSummaryManager",
  );
  let transactionSpec = namespace(transactionSummaryManager,
    "transactionSpec");

  transactionSummaryManager.update_direction = (showOrHide) =>
  {
    ...
    newTransaction.sendOrGet.toggleClass("displayNone", showOrHide !==
      "show");
    newTransaction.sendOrGet.text(transactionSpec.sendOrGetString);
  };
  transactionSummaryManager.update_targetAbsoluteAddress = (showOrHide) =>
  {
    transactionSpec.targetLedgerNodeName =
      targetAccountPhase.selectTargetLedgerSelect.val();
    transactionSpec.targetUserID =
      targetAccountPhase.selectTargetUserIDSelect.val();
    transactionSpec.targetAccountString =
      transactionSpec.targetLedgerNodeName + "/" + transactionSpec
      .targetUserID;

    newTransaction.targetAbsoluteAddress.toggleClass(
      "displayNone",
      showOrHide !== "show",
    );
    newTransaction.targetAbsoluteAddress.text(
      transactionSpec.targetAccountString,
    );
  };
  transactionSummaryManager.update_quantityAndDescription = (showOrHide) =>
  {
    transactionSpec.quantity =
      quantityAndDescriptionPhase.quantityOfCreditAutoNumeric.get();
    transactionSpec.quantityFormatted =
      quantityAndDescriptionPhase.quantityOfCreditAutoNumeric
      .getFormatted();

    transactionSpec.description = quantityAndDescriptionPhase.description
      .val();

    newTransaction.quantityOfCredit.toggleClass(
      "displayNone",
      showOrHide !== "show",
    );
    newTransaction.description.toggleClass(
      "displayNone",
      showOrHide !== "show",
    );
  ...
  let transactionSummaryPhase = (nodes.newTransaction
    .transactionSummaryPhase = {
      load: () =>
      {
          ...
          [
            ...
            [
              [
                ...
                [
                  [
                    "button.submitTransactionForValidation.buttonLeaf.buttonLeafSymetric",
        ...
        transactionSummaryPhase.submitTransactionForValidation.attr(
          "disabled",
          "disabled",
        );
        ...
        transactionSummaryPhase.submitTransactionForValidation.on(
          ...
          (event) =>
          {
            ...
            transactionSummaryPhase.submitTransactionForValidation
              .toggleClass(
                "validating",
                true,
              );
  ...
  let validationResponsePhase = (nodes.newTransaction
    .validationResponsePhase = {
      load: () =>
      {
            ...
            [
              ...
              [
                ...
                [
                  [
                    ".label",
                    "@Total Amount to " + transactionSpec.direction +
                    ":",
                  ],
              ...
              [
                ...
                [
                  [
                    "button.cancelTransaction.pauseOnLoad.buttonLeaf.buttonLeafSymetric.inputElement",
                    "@Cancel Transaction",
                  ],
                  [
                    "button.approveTransaction.pauseOnLoad.buttonLeaf.buttonLeafSymetric",
                    "@Approve & Confirm",
                  ],
        ...
        validationResponsePhase.cancelTransaction.on("click", (event) =>
        {
          validationResponsePhase.cancelTransaction.attr(
            "disabled",
            "disabled",
          );
          validationResponsePhase.approveTransaction.attr(
            "disabled",
            "disabled",
          );
          ...
          let eventToSend = {
            ...
            uiMajorStateCollectionParameters:
            {
              ...
              targetNodeName: "newTransaction",
              newTransactionID: validationResponsePhase
                .newTransactionID,
            },
          };
          ...
          O.create(
            [
              validationResponsePhase.phaseContent,
              ".cancellingTransaction.width100.textAlign_center",
        ...
        validationResponsePhase.approveTransaction.on("click", (event) =>
        {
          $(".validationResponsePhase button").attr("disabled",
            "disabled");
          ...
          O.create(
            [
              validationResponsePhase.phaseContent,
              ".approvingTransaction.width100.textAlign_center",
    ...
    load: () =>
    {
      ...
      O.create(
        [
          ...
          ".phaseContent." +
          phaseName +
          ".displayNone.flexAlignItemsCenter.flexJustifyContentCenter",
          [
      ...
      validationResponsePhase.cancelTransaction.on("click", (event) =>
      {
        validationResponsePhase.cancelTransaction.attr("disabled",
          "disabled");
        validationResponsePhase.approveTransaction.attr("disabled",
          "disabled");
